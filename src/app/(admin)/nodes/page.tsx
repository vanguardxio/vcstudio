...
export default function GlobalNodesCommandCenter() {
  ...
  // === FIX 5: Mobile performance — detect small screens, reduce heavy effects ===
  ...
  return (
    <LazyMotion features={domAnimation}>
      <div className="min-h-screen bg-slate-50 dark:bg-[#050B14] text-slate-900 dark:text-slate-50 font-sans selection:bg-amber-500/30 selection:text-amber-700 dark:selection:text-amber-400 transition-colors duration-500 relative overflow-hidden pb-24 md:pb-32">
        ...
        {/* Ambient GPU Glows — reduced blur on mobile */}
        <div className={`fixed top-[-10%] left-[-10%] w-[60%] h-[60%] rounded-full bg-slate-400/20 dark:bg-slate-500/10 ${isMobile ? "blur-[80px]" : "blur-[150px]"} pointer-events-none -z-10 transform-gpu will-change-transform`} />
        <div className={`fixed bottom-[-10%] right-[-10%] w-[60%] h-[60%] rounded-full bg-amber-500/15 dark:bg-amber-600/10 ${isMobile ? "blur-[80px]" : "blur-[150px]"} pointer-events-none -z-10 transform-gpu will-change-transform`} />
        ...
        <main className="max-w-[1600px] mx-auto px-4 md:px-6 lg:px-8 relative z-10 pt-16 md:pt-24">
          ...
          {/* ========================================================================= */}
          <m.header initial="hidden" animate="visible" variants={staggerContainer} className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-10 md:mb-12 relative">
            <div className="flex-1">
              <m.div variants={fadeUp} className="inline-flex items-center gap-3 px-4 py-2 border border-slate-300 dark:border-white/10 bg-white/60 dark:bg-slate-900/60 backdrop-blur-md rounded-full mb-8 shadow-sm w-fit">
              ...
              <m.h1 variants={fadeUp} className="text-5xl sm:text-6xl md:text-[5rem] font-black uppercase tracking-tighter drop-shadow-sm text-slate-900 dark:text-white leading-[0.95] mb-6">
              ...
              <m.p variants={fadeUp} className="text-base md:text-lg font-medium text-slate-600 dark:text-slate-400 max-w-xl leading-relaxed">
                Every 100ms of delay costs revenue. Every outage loses trust.
                <span className="text-slate-900 dark:text-white font-semibold"> This infrastructure protects both.</span>
              </m.p>
            </div>
            ...
            <m.div variants={fadeUp} className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
              <Link href="/dashboard/initiate" className="group relative flex items-center justify-between gap-6 px-6 py-4 bg-slate-900 dark:bg-amber-500 text-white dark:text-slate-950 rounded-2xl hover:bg-slate-800 dark:hover:bg-amber-400 transition-all shadow-xl overflow-hidden">
                ...
                <div>
                  <div className="text-[10px] font-mono uppercase tracking-widest font-bold opacity-70 mb-1">Application only</div>
                  <div className="text-sm font-black uppercase tracking-widest">Request Private Build</div>
                </div>
          ...
          {/* === TRUST + COMPETITIVE STRIP (moved below hero for hierarchy) === */}
          <m.div
            initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.4 }}
            className="mb-16 md:mb-20 flex flex-col sm:flex-row sm:items-center gap-6 sm:gap-0 sm:justify-between border-t border-b border-slate-200 dark:border-white/[0.06] py-5"
          >
            {/* Credibility badges */}
            <div className="flex flex-wrap gap-x-8 gap-y-2 text-[9px] md:text-[10px] font-mono uppercase tracking-widest font-bold text-slate-500 dark:text-slate-500">
              <span className="flex items-center gap-2"><CheckCircle2 className="w-3 h-3 text-amber-500"/> 99.999% uptime SLA</span>
              <span className="flex items-center gap-2"><CheckCircle2 className="w-3 h-3 text-amber-500"/> 142 global edge PoPs</span>
              <span className="flex items-center gap-2"><CheckCircle2 className="w-3 h-3 text-amber-500"/> Sub-20ms avg latency</span>
              <span className="flex items-center gap-2"><CheckCircle2 className="w-3 h-3 text-amber-500"/> Zero SaaS lock-in</span>
            </div>
            {/* Competitive pills */}
            <div className="flex flex-wrap gap-2 shrink-0">
              {["Faster than AWS CloudFront", "No Vercel lock-in", "You hold the keys"].map((label, i) => (
                <span key={i} className="flex items-center gap-1 text-[9px] font-mono font-bold uppercase tracking-widest px-2.5 py-1 rounded-full bg-slate-900/5 dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.07] text-slate-600 dark:text-slate-400">
                  <span className="w-1 h-1 rounded-full bg-amber-500 shrink-0" />{label}
                </span>
              ))}
            </div>
          </m.div>
            ...
            <m.div variants={fadeUp} className="w-full h-[500px] md:h-[650px] rounded-[2rem] md:rounded-[3rem] bg-slate-100/60 dark:bg-[#0A0F1C]/60 border border-slate-300 dark:border-white/10 backdrop-blur-3xl shadow-2xl relative overflow-hidden flex items-center justify-center">
              ...
              <div className="absolute bottom-6 md:bottom-10 left-6 md:left-10 z-20 backdrop-blur-2xl bg-white/80 dark:bg-[#050B14]/80 border border-slate-300 dark:border-white/10 rounded-2xl p-5 shadow-xl w-[220px]">
                ...
                <div className="flex flex-col gap-2.5">
                  <button className="flex items-center justify-between w-full text-[10px] font-bold uppercase tracking-widest px-3 py-2 bg-amber-500/10 hover:bg-amber-500/20 text-amber-700 dark:text-amber-400 rounded-lg border border-amber-500/20 transition-all active:scale-95">
                    Reroute Traffic <RefreshCw className="w-3 h-3" />
                  </button>
                  <button className="flex items-center justify-between w-full text-[10px] font-bold uppercase tracking-widest px-3 py-2 bg-slate-200 hover:bg-slate-300 dark:bg-white/5 dark:hover:bg-white/10 text-slate-700 dark:text-slate-300 rounded-lg transition-all active:scale-95 border border-transparent dark:border-white/5">
                    Simulate Failover <ShieldAlert className="w-3 h-3" />
                  </button>
                  <button className="flex items-center justify-between w-full text-[10px] font-bold uppercase tracking-widest px-3 py-2 bg-slate-200 hover:bg-slate-300 dark:bg-white/5 dark:hover:bg-white/10 text-slate-700 dark:text-slate-300 rounded-lg transition-all active:scale-95 border border-transparent dark:border-white/5">
                    Deploy Function <PlayCircle className="w-3 h-3" />
                  </button>
                </div>
              </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between items-center text-[10px] font-mono uppercase tracking-widest font-bold">
                        ...
                        <span className="text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
                          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" /> {activeNode.status}
                        </span>
                      </div>
                    </div>
                  </m.div>
              ...
              {/* ORBITAL MAP — parallax disabled on mobile */}
              <m.div style={isMobile ? {} : { y: yParallax }} className="absolute inset-0 flex items-center justify-center opacity-90 scale-[1.2] md:scale-[1.05]">
                <svg viewBox="0 0 1000 500" className="w-full h-full object-contain drop-shadow-[0_0_20px_rgba(245,158,11,0.15)]">
                  ...
                  {[
                    ...
                    <circle key={`ping-${i}`} cx={pt.x} cy={pt.y} r="2" fill="#f59e0b" className="pointer-events-none">
                      <animate attributeName="opacity" values="0;1;0" dur={`${1 + (i % 3)}s`} repeatCount="indefinite"/>
                    </circle>
                  ))}
                </svg>
              </m.div>
          ...
          <m.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={staggerContainer} className="mb-20 md:mb-28">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  { title: "AI SaaS at Global Scale", icon: BrainCircuit },
                  { title: "High-Ticket Conversion Funnels", icon: Target },
                  { title: "Financial Systems requiring 99.999%", icon: Activity },
                  { title: "Creator Platforms (Millions of Users)", icon: Users },
                ].map((useCase, i) => (
                  <m.div key={i} variants={fadeUp} className="p-6 rounded-2xl bg-white/60 dark:bg-slate-900/40 border border-slate-300 dark:border-white/10 backdrop-blur-xl flex flex-col items-center text-center gap-4">
                    <div className="p-3 rounded-full bg-slate-200 dark:bg-white/5 border border-slate-300 dark:border-white/10">
                      <useCase.icon className="w-5 h-5 text-amber-600 dark:text-amber-500" />
                    </div>
                    <h4 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wide">{useCase.title}</h4>
                  </m.div>
                ))}
             </div>
          </m.section>
             ...
             <div className="lg:col-span-5 flex flex-col gap-4 md:gap-6">
                <m.div variants={fadeUp} className="p-8 rounded-[2rem] bg-white/60 dark:bg-slate-900/40 border border-slate-300 dark:border-white/10 backdrop-blur-xl shadow-sm">
                   <div className="text-[10px] font-mono uppercase tracking-widest font-bold text-slate-500 mb-6 flex items-center gap-2">
                     <Trophy className="w-4 h-4 text-amber-500" /> Speed &amp; revenue
                   </div>
                   ...
                   <div className="p-4 rounded-xl bg-amber-500/5 border border-amber-500/15 mb-6">
                     <p className="text-[10px] font-mono font-bold uppercase tracking-widest text-amber-600 dark:text-amber-500">
                       What corporations spend six figures to build — delivered privately for founders.
                     </p>
                   </div>
                ...
                <m.div variants={fadeUp} className="p-6 rounded-[2rem] bg-white/60 dark:bg-slate-900/40 border border-slate-300 dark:border-white/10 backdrop-blur-xl shadow-sm flex-1">
                  ...
                  <div className="space-y-3">
                    {[
                      { r: "US East (N. Virginia)", t: "12ms", c: "w-[95%]" },
                      { r: "EU Central (Frankfurt)", t: "14ms", c: "w-[90%]" },
                      { r: "AP North (Tokyo)", t: "18ms", c: "w-[85%]" },
                      { r: "SA East (São Paulo)", t: "24ms", c: "w-[75%]" },
                    ].map((row, i) => (
                      <div key={i} className="flex flex-col gap-1.5">
                        <div className="flex justify-between text-xs font-bold text-slate-900 dark:text-white">
                          <span>{row.r}</span>
                          <span className="text-amber-600 dark:text-amber-500">{row.t}</span>
                        </div>
                        <div className="h-1.5 w-full bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                          <div className={`h-full bg-slate-900 dark:bg-amber-500 rounded-full ${row.c}`} />
                        </div>
                      </div>
                    ))}
                  </div>
             ...
             <m.div variants={fadeUp} className="lg:col-span-7 flex flex-col h-full min-h-[400px] rounded-[2rem] bg-slate-900 dark:bg-black/80 border border-slate-800 dark:border-white/10 backdrop-blur-3xl shadow-xl overflow-hidden relative">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-white/5">
                  <div className="flex items-center gap-2 text-[10px] font-mono uppercase tracking-widest font-bold text-white">
                    <Radio className="w-4 h-4 text-amber-500" /> Global Traffic Feed
                  </div>
                ...
                <div className="flex-1 p-6 font-mono text-[10px] md:text-xs flex flex-col justify-end gap-3 relative" ref={logAreaRef}>
                  ...
                  <div className="flex items-center gap-2 mt-2 text-amber-500 animate-pulse">
                    <span className="w-1.5 h-3.5 bg-amber-500 inline-block shadow-[0_0_8px_rgba(245,158,11,0.8)]" />
          ...
          <m.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={staggerContainer} className="mb-20 md:mb-28">
             <div className="text-center mb-10 max-w-3xl mx-auto">
                ...
                <p className="text-slate-600 dark:text-slate-400 font-medium text-base md:text-lg">Not shared hosting. Dedicated, isolated edge compute designed to handle infinite scale and hostile traffic — without breaking a sweat.</p>
             </div>
             <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                {[
                  { i: Shield, t: "Zero-Day Defense", d: "Enterprise WAF blocks malicious traffic at the network edge before it ever reaches your application logic." },
                  { i: Layers, t: "Infinite Elasticity", d: "Serverless functions scale from 0 to 100,000 requests per second automatically. You never pay for idle capacity." },
                  { i: Database, t: "Global State Sync", d: "Your customer data, AI memory, and configurations are replicated worldwide for instant access, everywhere." },
                ].map((feat, idx) => (
                  <m.div key={idx} variants={fadeUp} className="p-8 rounded-[2rem] bg-white/60 dark:bg-slate-900/40 border border-slate-300 dark:border-white/10 backdrop-blur-xl text-center group hover:border-amber-500/30 transition-colors">
                     <div className="w-16 h-16 mx-auto rounded-2xl bg-slate-100 dark:bg-white/5 border border-slate-300 dark:border-white/10 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                        <feat.i className="w-8 h-8 text-amber-600 dark:text-amber-500" />
                     </div>
                     <h3 className="text-lg font-black uppercase text-slate-900 dark:text-white mb-3">{feat.t}</h3>
                     <p className="text-slate-600 dark:text-slate-400 text-sm font-medium leading-relaxed">{feat.d}</p>
                  </m.div>
                ))}
             </div>
          </m.section>
          ...
          <m.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={fadeUp} className="mb-8 text-center relative z-20">
            <div className="p-10 md:p-20 rounded-[3rem] bg-slate-900 dark:bg-black border border-slate-800 dark:border-white/10 backdrop-blur-3xl shadow-[0_20px_60px_rgba(0,0,0,0.2)] dark:shadow-[0_20px_60px_rgba(0,0,0,0.6)] relative overflow-hidden group">
              <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-amber-500/10 via-slate-900/0 dark:via-black/0 to-transparent pointer-events-none" />
              <div className="absolute top-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-amber-500/50 to-transparent" />
              ...
              <Link href="/dashboard/initiate" className="inline-flex items-center gap-4 px-12 py-6 bg-amber-500 text-slate-950 font-black uppercase tracking-widest text-xs md:text-sm rounded-2xl hover:bg-amber-400 transition-all shadow-[0_15px_40px_rgba(245,158,11,0.3)] hover:shadow-[0_20px_50px_rgba(245,158,11,0.4)] hover:-translate-y-1">
                <Power className="w-5 h-5" /> Request Private Build
              </Link>
          ...
          <div className="max-w-[1600px] mx-auto px-6">
            <div className="flex flex-col items-center mb-16">
              <div className="w-12 h-12 border-2 border-slate-900 dark:border-amber-500 flex items-center justify-center relative mb-6 shadow-sm">
                <div className="w-3 h-3 bg-slate-900 dark:bg-amber-500 absolute top-0 left-0" />
              ...
              <span className="text-[10px] font-mono tracking-widest text-amber-700 dark:text-amber-500 font-bold uppercase mb-4">Sovereign DFY Systems</span>
              {/* === FIX 2: UPGRADED FOUNDER AUTHORITY === */}
              <div className="text-center space-y-1">
                <p className="text-xs font-medium text-slate-500 dark:text-slate-500">Personally engineered by</p>
                <p className="text-sm font-black tracking-widest text-slate-800 dark:text-white uppercase">Knowledge Rumhizha</p>
                <p className="text-[10px] font-mono tracking-widest text-amber-600 dark:text-amber-500 uppercase font-bold">Founder · Vaulltcore Infrastructure</p>
              </div>
            </div>

            <div className="flex flex-wrap justify-center items-center gap-8 mb-16">
              <a href="mailto:build@vaulltcore.com" className="text-[10px] md:text-xs font-mono font-bold tracking-widest uppercase text-slate-500 hover:text-amber-600 dark:hover:text-amber-500 transition-colors">
            ...
            <div className="w-full flex flex-col md:flex-row items-center justify-between border-t border-slate-300 dark:border-white/5 pt-8 gap-4">
              <span className="text-[9px] font-mono uppercase tracking-widest text-slate-500 text-center md:text-left">
                © {new Date().getFullYear()} Vaulltcore · Knowledge Rumhizha · Harare, Zimbabwe · Command Center Ready
              </span>
