...
import {
  LazyMotion, domAnimation, m, useScroll, useSpring, Variants, AnimatePresence, useTransform
} from "framer-motion";
import {
          ...
          <m.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={staggerContainer} className="mb-20 md:mb-28">
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                {[
                  ...
                  { i: Layers, t: "Infinite Elasticity", d: "Serverless functions automatically scale from 0 to 100,000 requests per second. You never pay for idle capacity." },
                  ...
                  <m.div key={idx} variants={fadeUp} className="p-8 rounded-[2rem] bg-white/60 dark:bg-slate-900/40 border border-slate-300 dark:border-white/10 backdrop-blur-xl text-center group hover:border-amber-500/30 transition-colors">
                     ...
                     <h3 className="text-xl font-black uppercase text-slate-900 dark:text-white mb-3">{feat.t}</h3>
                     <p className="text-slate-600 dark:text-slate-400 text-sm font-medium">{feat.d}</p>
                  </m.div>
                ))}
             </div>
          </m.section>
